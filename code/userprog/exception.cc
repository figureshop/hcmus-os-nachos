...
void solve_SC_Exit() {
	...
	kernel->machine->WriteRegister(2, SysExit(virtAddr));
	increasePC();
}

void solve_SC_Exec() {
	int virtAddr;
    virtAddr = GET_ARGUMENT(1);  // read program address
    char* programName = user2System(virtAddr);  // load into kernel
    if (programName == NULL) {
        DEBUG(dbgSys, "\nSC_EXEC: Cannot open the file! \n");
        ASSERT(false);
        kernel->machine->WriteRegister(2, -1);
        increasePC();
    }

    kernel->machine->WriteRegister(2, SysExec(programName));
    increasePC();
}

void solve_SC_Join() {
	int virtAddr = GET_ARGUMENT(1); // read program address
    kernel->machine->WriteRegister(2, SysJoin(virtAddr));
    increasePC();
}

void solve_SC_Open() {
	int virtAddr = GET_ARGUMENT(1); // get name address from register 4
    char* fileName = user2System(virtAddr);
    int type = GET_ARGUMENT(2); // get type from register 5
